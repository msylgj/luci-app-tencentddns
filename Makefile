...
define Build/Prepare
	$(foreach po,$(wildcard ${CURDIR}/lucisrc/i18n/*.po), \
...
define Package/luci-app-tencentddns/install
	...
	$(INSTALL_DATA) ./lucisrc/controller/*.lua $(1)/usr/lib/lua/luci/controller/
	$(INSTALL_DIR) $(1)/usr/lib/lua/luci/model/cbi/tencentddns
	$(INSTALL_DATA) ./lucisrc/model/cbi/*.lua $(1)/usr/lib/lua/luci/model/cbi/
	$(INSTALL_DIR) $(1)/etc/config
	$(INSTALL_DATA) ./root/etc/config/tencentddns $(1)/etc/config/tencentddns
	$(INSTALL_DIR) $(1)/etc/init.d
	$(INSTALL_BIN) ./root/etc/init.d/tencentddns $(1)/etc/init.d/tencentddns
	$(INSTALL_DIR) $(1)/etc/uci-defaults
	$(INSTALL_BIN) ./root/etc/uci-defaults/luci-tencentddns $(1)/etc/uci-defaults/luci-tencentddns
	$(INSTALL_DIR) $(1)/usr/sbin
	$(INSTALL_BIN) ./root/usr/sbin/tencentddns $(1)/usr/sbin/tencentddns
	$(INSTALL_DIR) $(1)/etc
	$(INSTALL_BIN) ./root/etc/tencentddnsupload $(1)/etc/tencentddnsupload
endef
